...
# model4 = lm(free4 ~ dates_free4)
# ipv4$free4 = ipv4$free4 + 908800
# m5 = lm(free4 ~ dates_free4, data = ipv4)
m5 = lm(free4 ~ poly(dates_free4, 1), data = ipv4)
# m5 = auto.arima(ts(ipv4$free4))
...
# Forecasting ----------------------------------------------------
secsday = 86400
future_horizon_days = 1000
future_dates_free4 = seq(dates_free4[1], dates_free4[1]+secsday*future_horizon_days, secsday)
...
# Plotting
plot(c(future_dates_free4, dates_free4), c(future_free4, pr2[,'fit']), col='green', lwd=1, xlab='', ylab='', xaxt="n", yaxt="n")
...
lines(dates_free4, free4, col='black', lwd=3, xlab='')
lines(c(future_dates_free4, dates_free4), numeric(length(c(future_dates_free4, dates_free4))), col='yellow', xlab='')
